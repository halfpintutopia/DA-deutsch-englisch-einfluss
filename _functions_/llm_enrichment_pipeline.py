    ...
    for i in tqdm(range(0, len(all_rows), batch_size), desc="LLM Enrichment"):
        ...
        for _, row in batch.iterrows():
            try:
                ...
                text = row["text"]
                loanwords = json.loads(row["loanwords"]) if isinstance(row["loanwords"], str) else row["loanwords"]

                excluded = detect_unwanted_loanwords(text, loanwords)
                refined = [w for w in loanwords if w not in excluded]

                enriched = {
                    "article_id": article_id,
                    "refined_loanwords": json.dumps(refined),
                    "excluded_loanwords": json.dumps(excluded),
